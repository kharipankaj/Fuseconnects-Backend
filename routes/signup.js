...
router.post('/', async (req, res) => {
  try {
    ...
    try {
      ...
      savedUser.verificationTokenHash = hashToken(verificationToken);
      savedUser.verificationExpires = new Date(Date.now() + 2 * 24 * 60 * 60 * 1000); // 2 days
      await savedUser.save();

      const emailResult = await sendVerificationEmail(savedUser.email, verificationToken, savedUser.username);
    ...
    // Do NOT auto-login: require email verification before issuing tokens
    if (process.env.NODE_ENV !== "production") {
      console.log(`✅ Signup successful for user: ${savedUser.username}, verification email sent`);
    }
    ...
    res.status(201).json({ message: 'Signup successful. Verification email sent', user: {
      id: savedUser._id,
